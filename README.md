...
```bash
usage: skill_competencies_converter [-h] [--google-sheet GOOGLE_SHEET] [--legacy-csv LEGACY_CSV] [--output-path OUTPUT_PATH] [--print-output | --no-print-output]
...
options:
  ...
  --google-sheet GOOGLE_SHEET
                        Google Sheet ID. Processes full framework based on assumed sheet names.
  --legacy-csv LEGACY_CSV
                        CSV filepath for V1 of framework structure.
...
Examples:
    ...
    skill_competencies_converter --google-sheet <google_sheet_id>
    skill_competencies_converter --legacy-csv <csv_file>
    skill_competencies_converter --legacy-csv <csv_file> --output-path <file-name>
    skill_competencies_converter --google-sheet <google_sheet_id> --print-output
```
...
```bash
skill_competencies_converter --legacy-csv data/data.csv
```
...
```bash
skill_competencies_converter --google-sheet 1M8f9luJrc3w4rz4lD-cEpJUfi6qigh9PBk7Fm5sq_jQ --output-path framework.json
```

This will output multiple files, one for each table, named `framework-<table-name>.json`
...
```bash
python3 src/skill_competencies_converter/convert_skills.py --legacy-csv data/data.csv --output-path framework.json
```
```bash
python3 src/skill_competencies_converter/create_user_profile.py --legacy-csv data/data.csv --profile-name "RSE Team Lead profile" --output-path "data/profile.json"
```

#### Using a public Google sheet with data

```bash
python3 src/skill_competencies_converter/convert_skills.py --google-sheet 1M8f9luJrc3w4rz4lD-cEpJUfi6qigh9PBk7Fm5sq_jQ --output-path framework.json
```

```bash
python3 src/skill_competencies_converter/convert_skills.py --google-sheet 1umVxBzuZGDgins6XqJwuGVf6BK_DgjknhuwDQpNwyjo --profile-name "RSE Team Lead profile" --output-path "data/profile.json"
```
